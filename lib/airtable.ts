...
// This prevents Airtable from requiring API keys during build time
let baseInstance: Airtable.Base | null = null;

function getBase(): Airtable.Base {
  if (!baseInstance) {
    // Validate env vars at runtime (when actually needed)
    ...
    baseInstance = airtable.base(env.AIRTABLE_BASE_ID);
  }
  return baseInstance;
}
...
export async function getAllApps(): Promise<App[]> {
  try {
    const records = await getBase()(TABLE_NAME)
      .select({
        filterByFormula: '{Status} = "Published"',
      })
      .all();

    return records.map((record) => mapAirtableToApp(record as unknown as AirtableAppRecord));
  } catch (error) {
...
export async function getAppBySlug(slug: string): Promise<App | null> {
  try {
    ...
    return mapAirtableToApp(records[0] as unknown as AirtableAppRecord);
  } catch (error) {
...
export async function createAppSubmission(submission: AppSubmission): Promise<boolean> {
  try {
    await getBase()(TABLE_NAME).create([
      {
        fields: {
